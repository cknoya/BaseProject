...
import com.aigame.basecore.job.config.Priority;
import com.aigame.basecore.job.jobs.AsyncJob;
...
public class JobManagerUtils {
	...
	public static void init() {
		Configuration.Builder builder = new Configuration.Builder()
				.minConsumerCount(CPU_COUNT)// always keep at least one consumer
											// alive
				.maxConsumerCount(CPU_COUNT * 2 + 1)// up to CPU_COUNT * 2 + 1
													// consumers at a time
	 ...
	 *            任务优先级
	 * @param jobTag
	 *            任务名 在DEBUG模式下执行该任务的线程会动态变更为这个名字 执行完毕后名字会还原。 同时可以用来查询没有执行的任务列表
	 * @return
	 */
	public static AsyncJob postPriority(Runnable runnable, int priority,
			String jobTag) {
		if (runnable != null) {
	 ...
	 * @return
	 */
	public static AsyncJob post(final Runnable runnable, int priority,
			long delay, String groupId, String jobTag) {
		if (runnable != null) {
			AsyncJob<Object, Object> asyncJob = new AsyncJob<Object, Object>(
					Object.class) {
				@Override
