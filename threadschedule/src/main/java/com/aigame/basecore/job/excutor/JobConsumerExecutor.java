package com.aigame.basecore.job.excutor;
...
public class JobConsumerExecutor {
    ...
    private void addConsumer() {
        if (JobLog.isDebug()) {
            JobLog.d("adding another consumer");
        }
        synchronized (threadGroup) {
    ...
    private boolean isAboveLoadFactor(boolean inConsumerThread) {
        synchronized (threadGroup) {
            ...
            if (JobLog.isDebug()) {
                JobLog.d("%s: load factor check. %s = (%d < %d)|| (%d * %d < %d + %d). consumer thread: %s", Thread.currentThread().getName(), res,
                        consumerCnt, minConsumerSize,
        ...
        public void run() {
            ...
            do {
                ...
                try {
                    if (JobLog.isDebug()) {
                        if (didRunOnce == false) {
                            JobLog.d("starting consumer %s", Thread.currentThread().getName());
                            didRunOnce = true;
                        } else {
                            JobLog.d("re-running consumer %s", Thread.currentThread().getName());
                        }
                    }
                    JobHolder nextJob;
                    FutureJob job;
                    ...
                    if (JobLog.isDebug()) {
                        if (canDie) {
                            JobLog.d("finishing consumer %s", Thread.currentThread().getName());
                        } else {
                            JobLog.d("didn't allow me to die, re-running %s", Thread.currentThread().getName());
                        }
                    }
                }
            } while (!canDie);
