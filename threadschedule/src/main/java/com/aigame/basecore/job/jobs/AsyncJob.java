...
/**
 * 需要回调功能的任务 与{@link android.os.AsyncTask}用法相同
 * 
 */
public abstract class AsyncJob<RequestParams, Result> extends
		Job<RequestParams, Result> {

	/**
	 * @param resultClassType
	 *            后台线程返回值类型，用来检测泛型对象是否与返回一致，如果不传，或者与返回泛型不一致 则回调方法参数为空对象。
	 */
	 ...
	 */
	public final AsyncJob<RequestParams, Result> ensureToMain(
