...
import com.aigame.basecore.job.handler.DefaultJobHandler;
import com.aigame.basecore.job.handler.IJobHandler;
import com.aigame.basecore.job.handler.JobHolder;
import com.aigame.basecore.job.helper.JobLog;
...
public abstract class BaseJob<RequestParams, Result> {
	...
	/**
	 * called when the job is added to disk and committed. this means job will
	 * eventually run. this is a good time to update local database and dispatch
	 * events Changes to this class will not be preserved if your job is
	 * persistent !!! Also, if your app crashes right after adding the job,
	 * {@code onRun} might be called without an {@code onAdded} call
	 */
	public void onAdded() {
	}

	/**
	 * The actual method that should to the work It should finish w/o any
	 * exception. If it throws any exception, {@code shouldReRunOnThrowable}
	 * will be called to decide either to dismiss the job or re-run it.
	 * 
	 * @throws Throwable
	 */
	...
	final public void onCallback(Object object) {
		...
		if (object != null) {
			if (resultClassType != null
					&& resultClassType.isAssignableFrom(object.getClass())) {
	 ...
	 * return true if you want to run your job again, return false if you want
	 * to dismiss it. If you return false, onCancel will be called.
	 */
	...
	public final JobHolder.JobCallbackResult safeRun(int currentRunCount) {
		...
		if (JobLog.isDebug()) {
			JobLog.d("running job %s", this.getClass().getSimpleName());
		}
		...
		try {
			...
			result.isSafe = true;
			if (JobLog.isDebug()) {
				JobLog.d("finished job %s", this.getClass().getSimpleName());
			}
		} catch (Throwable t) {
			failed = true;
			JobLog.e(t, "error while executing job");
			reRun = currentRunCount < getRetryLimit();
			if (reRun) {
				try {
					reRun = shouldReRunOnThrowable(t);
				} catch (Throwable t2) {
					JobLog.e(t2, "shouldReRunOnThrowable did throw an exception");
				}
			} else {
				t.printStackTrace();
			}
	...
	/**
	 * before each run, JobManager sets this number. Might be useful for the
	 * {@link BaseJob#onRun(Object[])} ()} method
	 * 
	 * @return
	 */
	...
	/**
	 * Some jobs may require being run synchronously. For instance, if it is a
	 * job like sending a comment, we should never run them in parallel (unless
	 * they are being sent to different conversations). By assigning same
	 * groupId to jobs, you can ensure that that type of jobs will be run in the
	 * order they were given (if their priority is the same).
	 * 
	 * @return
	 */
	...
	/**
	 * By default, jobs will be retried {@code DEFAULT_RETRY_LIMIT} times. If
	 * job fails this many times, onCancel will be called w/o calling
	 * {@code shouldReRunOnThrowable}
	 * 
	 * @return
	 */
