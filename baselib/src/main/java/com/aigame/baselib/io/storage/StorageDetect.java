...
class StorageDetect {
    ...
    public static final String FINGERPRINT_DIRECTORY = File.separator + ".fingerprint_core" + File
            .separator;
    ...
    private static void createfingerprint(Context context) {
        ...
        String fingerName = SharedPreferencesFactory.get(context, "default_sd_fingerprint", "", StorageCheckor.SPNAME_STORAGE);
        ...
        if ((TextUtils.isEmpty(fingerName)) || (!fingerFile.exists())) {
            ...
            try {
                ...
                SharedPreferencesFactory.set(context, "default_sd_fingerprint", fingerNameNew, StorageCheckor.SPNAME_STORAGE);
                return;
            } catch (IOException e) {
                ExceptionUtils.printStackTrace(e);
    ...
    private static void filterStorageList(ArrayList<StorageItem> paramArrayList, Context context) {
        ...
        String fingerprint = SharedPreferencesFactory.get(context, "default_sd_fingerprint", "",
                StorageCheckor.SPNAME_STORAGE);
    ...
    /**
     * 需要在异步线程调用
     * @param mContext 全局上下文
     * @return storageItem列表
     */
    public static ArrayList<StorageItem> getStorageList(Context mContext)
    {
        boolean scanSDDoubleAndMerge = true;

        if (scanSDDoubleAndMerge) {
