...
class Environment4 {
    ...
    public static void initDevices(Context context) {
        ...
        try {
            ...
            } // bei Devices nur wenn zusätzlich
    ...
    public static class Device extends File {
        ...
        Device(Object storage, Context context) throws NoSuchMethodException,
                InvocationTargetException, IllegalAccessException {
            super((String) storage.getClass().getMethod("getPath").invoke(storage));
            for (Method m : storage.getClass().getMethods()) {
                if (m.getName().equals("getUserLabel") && m.getParameterTypes().length == 0 && m
                        .getReturnType() == String.class) {
                    mUserLabel = (String) m.invoke(storage); // ab Android 4.4
                }
                if (m.getName().equals("getUuid") && m.getParameterTypes().length == 0 && m
                        .getReturnType() == String.class) {
                ...
                if (m.getName().equals("isRemovable") && m.getParameterTypes().length == 0 && m
                        .getReturnType() == boolean.class) {
                    mRemovable = (Boolean) m.invoke(storage); // ab Android 4.0
                }
                if (m.getName().equals("isPrimary") && m.getParameterTypes().length == 0 && m
                        .getReturnType() == boolean.class) {
                    mPrimary = (Boolean) m.invoke(storage); // ab Android 4.2
                }
                if (m.getName().equals("isEmulated") && m.getParameterTypes().length == 0 && m
                        .getReturnType() == boolean.class) {
            ...
            try {
                if (mRemovable || mState == null) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        StorageManager sm = (StorageManager) context.getSystemService(Context
                                .STORAGE_SERVICE);
                        try {
                            ...
                            DebugLog.w(TAG, "StorageManager-->getVolumeState not found, " +
                                    "reflection failed");
                            ...
                            if (canRead() && getTotalSpace() > 0) {
                                ...
                                // nicht lesbar, keine Größe aber noch MOUNTED || oder ungesetzt
                                // => UNKNOWN
