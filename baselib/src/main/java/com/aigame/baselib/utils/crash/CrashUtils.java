...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = LifeCycleUtils.getApp()
                    .getPackageManager()
                    .getPackageInfo(LifeCycleUtils.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && LifeCycleUtils.getApp().getExternalCacheDir() != null) {
            defaultDir = LifeCycleUtils.getApp().getExternalCacheDir() + FILE_SEP + "crash" +
                    FILE_SEP;
        } else {
            defaultDir = LifeCycleUtils.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
